...
const GAME_ID = 4; // Identifiant du jeu GameCinema1

@Injectable()
export class AdminService {
    // ✅ Génère un GameCinema2Day pour chaque jour entre `startDate` et `endDate`
    ...
    async getGameDaysStatus(month: string) {
        ...
        const existingDays = await prisma.gameCinema2Days.findMany({
            where: {
                date: {
                    gte: start,
                    lte: end,
    ...
    async regenerateGameDay(date: string) {
        ...
        await prisma.gameCinema2Days.deleteMany({
            where: { date: new Date(date) },
        });

        // Sélectionner un film aléatoire qui n'a pas été utilisé récemment
        const recentMovies = await prisma.gameCinema2Days.findMany({
            orderBy: { date: "desc" },
        ...
        await prisma.gameCinema2Days.create({
            data: {
                date: new Date(date),

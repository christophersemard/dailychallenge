...
describe("AdminService", () => {
    ...
    describe("generateGameDays", () => {
        it("should call regenerateGameDay for each date", async () => {
            ...
            const result = await service.generateGameDays("2024-03-01", "2024-03-03");
    ...
    describe("getGameDaysStatus", () => {
        it("should return upcoming and missing days", async () => {
            ...
            jest.spyOn(prisma.gameCinema1Days, "findMany").mockResolvedValue(existingDates as any);
            ...
            expect(result.missingDays).toContain("2024-03-03"); // Juste un exemple
        });
    });

    describe("regenerateGameDay", () => {
        it("should throw error if no movie is available", async () => {
            jest.spyOn(prisma.gameCinema1Days, "deleteMany").mockResolvedValue({} as any);
            jest.spyOn(prisma.gameCinema1Days, "findMany").mockResolvedValue([] as any);
            jest.spyOn(prisma.dataMovie, "findMany").mockResolvedValue([] as any);

            await expect(service.regenerateGameDay("2024-03-01")).rejects.toThrow(
                "Pas assez de films disponibles !"
            );
        });

        it("should create a new game day with a random movie", async () => {
            jest.spyOn(prisma.gameCinema1Days, "deleteMany").mockResolvedValue({} as any);
            jest.spyOn(prisma.gameCinema1Days, "findMany").mockResolvedValue([
                { movieId: 1 },
                { movieId: 2 },
            ] as any);
            ...
            const createSpy = jest.spyOn(prisma.gameCinema1Days, "create").mockResolvedValue({} as any);

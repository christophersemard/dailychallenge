...
describe("GameController", () => {
    ...
    it("should call submitGuess with correct parameters", async () => {
        ...
        expect(service.submitGuess).toHaveBeenCalledWith(1, "123", "2024-03-24");
        expect(result).toBe("submit_result");
    });

    it("should call searchMovie with correct query", async () => {
        const payload = { query: "inception" };
        mockGameService.searchMovie.mockResolvedValue(["movie1"]);

        const result = await controller.searchMovie(payload);
        expect(service.searchMovie).toHaveBeenCalledWith("inception");

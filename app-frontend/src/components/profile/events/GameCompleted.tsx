...
import { UserEvent, UserMe, UserPublic } from "@/types/user.types";
...
export default function GameCompleted({
    ...
    const getPathGameOfTheDay = () => {
        ...
        if (
            new Date(event.gameResult.date).toLocaleDateString() ===
            new Date().toLocaleDateString()
        ) {
            return `/jeu/${event.gameResult.game!.path}`;
        }
        // Sinon, on renvoie le path du jeu de l'event avec la date en paramètre
        return `/jeu/${event.gameResult.game!.path}/${getDateStr(
            new Date(event.gameResult.date)
        )}`;
    };
                    ...
                    <div className="flex flex-col">
                        <div className="flex items-center gap-1 gap-y-0  flex-wrap">
                            ...
                            <span className="font-bold">
                                {new Date(
                                    event.gameResult.date
                                ).toLocaleDateString()}
                            </span>
                            <span className="text-sm ">sur</span>
                            <Link
                                href={`/jeu/${event.gameResult.game!.path}`}
                            ...
                            <div className="text-sm font-bold">
                                (+{event.gameResult.score} pts et{" "}
                                {event.gameResult.xpGained} xp)
                            </div>
                        </div>

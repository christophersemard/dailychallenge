...
export default function GameLeaderboard({ leaderboardId, userId, color }: Props) {
                ...
                <div className="flex gap-2 text-xs">
                    {(["global", "amis"] as Tab[]).map((value) => (
                        <button
                            ...
                            className={clsx("cursor-pointer px-1.5 py-1 rounded border-2 border-white hover:border-background", {
            ...
            <ul className="space-y-0 text-sm mt-3.5">
                {loading && entries.length === 0 ? (
                    Array.from({ length: 5 }).map((_, i) => (
                        <li key={i} className="h-10 bg-background rounded animate-pulse" />
                    ))
                ) : entries.length === 0 ? (
                    ...
                    entries.map((entry, i) => {
                        ...
                        return (
                            ...
                            >
                                {/* Position */}
                                <div className={clsx("text-center font-black text-base me-2", getRankClass(i, offset))}>
                                    {offset + i + 1}
                                </div>

                                <Link className="flex flex-nowrap gap-2 items-center" href={`/profil/${entry.user.id}`}>
                                    <Image
                                        height={32}
                                        width={32}
                                        src={entry.user.avatar || `/assets/avatar/avatar-default-${Math.floor(Math.random() * 7 + 1)}.png`}
                                ...
                                <div className="flex justify-end items-center pe-2">
                                    <OutlineText
                                        ...
                                        size="sm"
                                    />
                                    <span className="text-muted-foreground text-xs ms-1 mb-0.5">pts</span>
                                </div>
                            </li>
                        )
                    })
